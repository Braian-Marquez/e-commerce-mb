...
public class UserAuthController {
	...
	public ResponseEntity<?> login(@RequestBody  AuthenticationRequest authenticationRequest)
		...
		if (userEntity != null) {
			...
			Customer customer = perfilRepository.getByUserId(userEntity.getId()).orElse(new Customer());
			AuthenticationResponse auth = new AuthenticationResponse();
			auth.setToken(jwtToken);
			auth.setIdCustomer(customer.getId());
			auth.setIdUser(userEntity.getId());
			auth.setName(customer.getFirstName());
