			...
			return WebClient.builder().baseUrl(getUrl() + "/user/v1")
					...
					.bodyToMono(TokenValidate.class).flatMap(response -> {
						if (response != null) {
                            ...
                            Long profileId = response.getIdProfile();
                            exchange.getRequest().mutate().header("profile-id", String.valueOf(profileId));
      
							if(response.getRole().isEmpty()&&!"ROLE_USUARIO".equals(response.getRole())  
									&& (!"ROLE_ADMIN".equals(response.getRole()))){
								try {
